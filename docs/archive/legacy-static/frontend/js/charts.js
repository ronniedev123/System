...
function getFilteredMembers(members) {
    ...
    return (members || []).filter((m) => normalizeDepartment(m.department) === activeDepartment);
}
...
function updateDepartmentTable(members) {
    ...
    departments.forEach((dept) => {
        ...
        tr.innerHTML = `
            <td style="border: 1px solid #ddd; padding: 12px; ${isActive ? 'font-weight: bold;' : ''}">${dept}</td>
            <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">${counts[dept] || 0}</td>
        `;
    ...
    totalRow.innerHTML = `
        <td style="border: 1px solid #ddd; padding: 12px; font-weight: bold;">Whole Church (Master)</td>
        <td style="border: 1px solid #ddd; padding: 12px; text-align: center; font-weight: bold;">${(members || []).length}</td>
    `;
    ...
    try {
        ...
        sortedMonths.forEach(k => {
            ...
            tr.innerHTML = `
                ...
                <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">${filteredMembers.length}</td>
