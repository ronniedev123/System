...
window.deleteDonation = async function(id){
    ...
    try{
        const res = await fetch(`http://localhost:3000/api/donations/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
...
window.editDonation = async function(id){
    try{
        const getRes = await fetch(`http://localhost:3000/api/donations/${id}`, { headers: { Authorization: `Bearer ${token}` } });
...
if(donationForm){
    donationForm.addEventListener("submit", async e=>{
        ...
        try{
            const res = await fetch("http://localhost:3000/api/donations/add", {
                method:"POST",
                headers: { "Content-Type":"application/json", "Authorization": `Bearer ${token}` },
